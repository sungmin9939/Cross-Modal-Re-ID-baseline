...
from utils import *
from loss import OriTripletLoss, TripletLoss_WRT, two_level_Proxy_Anchor
...
if dataset == 'sysu':
    # training set
    trainset = SYSUData(data_path, transform=transform_train, transform_gray=transform_gray)
...
if args.method == 'agw':
    ...
    criterion_two = two_level_Proxy_Anchor(512, 100)
else:
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, input3, label1, label2, label3) in enumerate(trainloader):
        ...
        input3 = Variable(input3.to(device))
