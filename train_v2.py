...
from model import embed_net_v2
from utils import *
...
if args.method =='base':
    net = embed_net_v2(n_class, no_local= 'off', gm_pool =  'off', arch=args.arch, syn=args.syn, local_attn=args.local_attn, proxy=args.proxy)
else:
    net = embed_net_v2(n_class, no_local= 'on', gm_pool = 'on', arch=args.arch, syn=args.syn, local_attn=args.local_attn, proxy=args.proxy)
net.to(device)
...
if args.optim == 'Adam':
    param_groups = [
        {'params': list(set(net.module.parameters())
                        .difference(set(net.module.mono_linear.parameters())
                                    # .union(net.module.ir_linear.parameters())
                                    # .union(net.module.rgb_linear.parameters())
                                    ))},
        # {'params': net.module.rgb_linear.parameters(), 'lr':float(args.lr) * 1},
        # {'params': net.module.ir_linear.parameters(), 'lr':float(args.lr) * 1},
        {'params': net.module.mono_linear.parameters(), 'lr':float(args.lr) * 1},
    ]
...
def train(epoch):
    ...
    for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
        ...
        mono_pred = net(input1, input2)
        
        # loss_rgb = criterion_id(rgb_pred, labels[:batch_size])
        # loss_ir = criterion_id(ir_pred, labels[batch_size:])
        loss_mono = criterion_id(mono_pred, labels)
        
        # rgb_proxy_loss.update(loss_rgb.item(), batch_size)
        # ir_proxy_loss.update(loss_ir.item(), batch_size)
        mono_proxy_loss.update(loss_mono.item(), batch_size*2)
        
        loss = loss_mono
        
        train_loss.update(loss.item(), batch_size*4)
        ...
        if batch_idx % 50 == 0:
            ...
            print('Epoch: [{}][{}/{}] '
                  ...
                  'mono_proxy_loss: {mono_proxy_loss.val:.4f} ({mono_proxy_loss.avg:.4f}) '   
                  .format(
                epoch, batch_idx, len(trainloader), current_lr, train_loss=train_loss, 
                rgb_proxy_loss=rgb_proxy_loss, ir_proxy_loss=ir_proxy_loss, mono_proxy_loss=mono_proxy_loss))
    ...
    wandb.log({'mono_proxy_loss': mono_proxy_loss.avg}, step=epoch)
